...
# Fixed grid size
GRID_SIZE = 232  # 28x28 grid = 784 slots
...
def marketDaysImage():
    ...
    for date in all_dates:
        pct_changes = [token_data[sym].loc[date, "pct_change"] for sym in tokens]
        if len(pct_changes) < total_cells:
            pct_changes += [0] * (total_cells - len(pct_changes))
        matrix = np.array(pct_changes).reshape((GRID_SIZE, GRID_SIZE))
...
def train_pred_rnn(period_matrices, all_dates, num_epochs=1000, batch_size=32, model_path='pred_rnn_model.pth'):
    ...
    matrices = np.array([period_matrices[date] for date in all_dates])[:, np.newaxis, :, :]
    sequences = [matrices[i:i+11] for i in range(len(matrices)-10)]
    ...
    model = PredRNNpp(4, 128, 1, 1, GRID_SIZE).to(device)
    ...
    if os.path.exists(model_path):
        try:
            ...
            print("Starting training from scratch (checkpoint mismatch).")
    ...
    for epoch in range(num_epochs):
        ...
        for batch in dataloader:
            ...
            loss = criterion(outputs[:, -1, 0], targets.squeeze(1))
            loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    model = train_pred_rnn(matrices, dates, num_epochs=1, batch_size=1, model_path='pred_rnn_model.pth')
